...
/**
 * 支持刷新+加载更多的SwipeRefresh控件
 * @author alafighting 16/01/27 封装简化，开放接口
 * @author Aspsine 2015/8/13
 */
public class SwipeRefreshExpandLayout extends ViewGroup {

    private static final String TAG = SwipeRefreshExpandLayout.class.getSimpleName();
     ...
     */
    private float mRefreshFinalDragOffset = CircleLoadLayout.DEFAULT_FINALDRAG_OFFSET;

    /**
     * <b>ATTRIBUTE:</b>
     * the max value of bottom offset
     */
    private float mLoadMoreFinalDragOffset = CircleLoadLayout.DEFAULT_FINALDRAG_OFFSET;
    ...
    public SwipeRefreshExpandLayout(Context context) {
        this(context, null);
    }

    public SwipeRefreshExpandLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SwipeRefreshExpandLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        setRefreshEnabled(true);
        setLoadMoreEnabled(true);
        setSwipeStyle(STYLE.ABOVE);
        setRefreshFinalDragOffset(0);
        setLoadMoreFinalDragOffset(0);
        setRefreshTriggerOffset(0);
        setLoadMoreTriggerOffset(0);
        setReleaseToRefreshingScrollingDuration(DEFAULT_RELEASE_TO_REFRESHING_SCROLLING_DURATION);
        setRefreshCompleteDelayDuration(DEFAULT_REFRESH_COMPLETE_DELAY_DURATION);
        setRefreshCompleteToDefaultScrollingDuration(DEFAULT_REFRESH_COMPLETE_TO_DEFAULT_SCROLLING_DURATION);
        setDefaultToRefreshingScrollingDuration(DEFAULT_DEFAULT_TO_REFRESHING_SCROLLING_DURATION);
        setReleaseToLoadingMoreScrollingDuration(DEFAULT_RELEASE_TO_LOADING_MORE_SCROLLING_DURATION);
        setLoadMoreCompleteDelayDuration(DEFAULT_LOAD_MORE_COMPLETE_DELAY_DURATION);
        setLoadMoreCompleteToDefaultScrollingDuration(DEFAULT_LOAD_MORE_COMPLETE_TO_DEFAULT_SCROLLING_DURATION);
        setDefaultToLoadingMoreScrollingDuration(DEFAULT_DEFAULT_TO_LOADING_MORE_SCROLLING_DURATION);
    ...
    protected ViewGroup.LayoutParams generateDefaultLayoutParams() {
        return new SwipeRefreshExpandLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
    }
    ...
    protected ViewGroup.LayoutParams generateLayoutParams(ViewGroup.LayoutParams p) {
        return new SwipeRefreshExpandLayout.LayoutParams(p);
    }
    ...
    public ViewGroup.LayoutParams generateLayoutParams(AttributeSet attrs) {
        return new SwipeRefreshExpandLayout.LayoutParams(getContext(), attrs);
    }
    ...
    protected void onFinishInflate() {
        ...
        if (childNum == 0) {
            ...
            mHeaderView = findHeaderView();
            mFooterView = findFooterView();
            mTargetView = findTargetView();
        } else {
    ...
    protected <T extends View & SwipeTrigger & SwipeRefreshTrigger> T findHeaderView() {
//        return (T) getChildAt(0);
        CircleLoadLayout headerView = createCircleLoadLayout();
        addView(headerView, 0);
        return (T) headerView;
    }
    protected <T extends View & SwipeTrigger & SwipeLoadMoreTrigger> T findFooterView() {
//        return (T) getChildAt(2);
        CircleLoadLayout footerView = createCircleLoadLayout();
        footerView.setGravity(CircleLoadLayout.Gravity.BOTTOM);
        addView(footerView);
        return (T) footerView;
    }
     ...
     * @return
     */
    private CircleLoadLayout createCircleLoadLayout() {
        CircleLoadLayout footerView = new CircleLoadLayout(getContext());
        footerView.setLayoutParams(createLayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, CircleLoadLayout.DEFAULT_FINALOVER_OFFSET));
        footerView.setBackgroundColor(Color.TRANSPARENT);

        footerView.addView(createProgressView());
        return footerView;
    }
    private ViewGroup.LayoutParams createLayoutParams(int width, int height) {
        return new MarginLayoutParams(width, height);
    }
    private View createProgressView() {
        CircleImageView progressView = new CircleImageView(getContext());
        progressView.setShowArrow(true);
        int drawableSize = CircleImageView.DEFAULT_DRAWABLE_SIZE;
        FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(drawableSize, drawableSize);
